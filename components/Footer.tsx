...
import { Instagram, Music2, Youtube, MapPin, Phone, Mail, Clock } from "lucide-react";
...
const Footer = () => {
  ...
  const socialLinks = [
    { icon: Instagram, href: "https://instagram.com/aceleraeadparceiros", name: "Instagram" },
  ...
  const contactItems = [
    {
      icon: MapPin,
      lines: ["Luiz Rodrigues dos Santos, 44", "Todos os Santos - Coronel Fabriciano/MG", "CEP: 35170-061"],
      href: "http://googleusercontent.com/maps.google.com/2"
    },
    { icon: Phone, lines: ["(31) 97329-6330"], href: "tel:+5531973296330" },
    { icon: Mail, lines: ["gerencial@colegioaceleraead.com.br"], href: "mailto:gerencial@colegioaceleraead.com.br" },
  ...
  return (
    ...
    >
          ...
          <motion.div variants={columnVariants}>
            <Link href="/" className="flex items-center space-x-3 mb-4">
              <div className="w-12 h-12 flex items-center justify-center bg-white rounded-lg">
                <Image src="/Acelera EAD.png" alt="Faculdade Marinho Logo" width={32} height={32} />
              </div>
              <span className="text-2xl font-bold">Acelera EAD</span>
            </Link>
            <p className="text-gray-400 mb-6 leading-relaxed">
              Transformando vidas através da educação de qualidade há mais de 18 anos.
            </p>
                ...
                <li key={index}>
                  <a href={item.href} target="_blank" rel="noopener noreferrer" className="flex items-start gap-3 group">
                    ...
                    <div className="text-gray-300 group-hover:text-white transition-colors duration-300">
                      {item.lines.map((line, i) => <span key={i} className="block">{line}</span>)}
                    </div>
                  </a>
                </li>
              ))}
        ...
        <div className="border-t border-white/10 mt-12 pt-6">
          <div className="flex flex-col md:flex-row justify-between items-center text-center md:text-left">
            <p className="text-gray-400 text-sm mb-4 md:mb-0">
              © {new Date().getFullYear()} Acelera EAD. Todos os direitos reservados.
            </p>
            <div className="flex space-x-6 text-sm text-gray-400">
              <button onClick={() => handleOpenModal('privacidade')} className="hover:text-primary transition-colors">Política de Privacidade</button>
              <button onClick={() => handleOpenModal('termos')} className="hover:text-primary transition-colors">Termos de Uso</button>
              <button onClick={() => handleOpenModal('cookies')} className="hover:text-primary transition-colors">Cookies</button>
            </div>
          </div>
        </div>
      </div>
      ...
      {modalAberto === 'termos' && ( <ModalTermosDeUso onClose={handleCloseModal} /> )}
    </motion.footer>
  );
};
